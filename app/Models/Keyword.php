...
class Keyword extends Model
{
	...
	{
		return $this->belongsToMany('\pdam\Models\Medicine', 'keyword_medicine', 'keyword_id', 'medicine_id');
	}

	public function assignMedicine($medicine)
	{
		if (is_string($medicine)) {
            $medicine = \pdam\Models\Medicine::where('id', $medicine)->orWhere('nama', $medicine)->first();
        }
	...
	{
		return $this->belongsToMany('\pdam\Models\Info', 'keyword_info', 'keyword_id', 'info_id');
	}

	public function assignInfo($info)
	{
		if (is_string($info)) {
            $info = \pdam\Models\Info::where('id', $info)->orWhere('nama', $info)->first();
        }
